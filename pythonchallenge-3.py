...
code='''kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
...
PBuijeoTSpsVLaOGuLVjMZXkBvVXwUuHfBihziiavGSYofPNeKsTXruMUumRRPQJzvSzJkKbtSipiqBd'''.replace('\n','')

upper='ABCDEFGHIJKLMNOPQRSTUVWXYZ'
lower=upper.lower()

for i in xrange(4,len(code)-4):
    if (code[i-3] in upper) and (code[i-2] in upper) and (code[i-1] in upper):
        if (code[i+3] in upper) and (code[i+2] in upper) and (code[i+1] in upper):
            if (code[i] in lower) and (code[i+4] in lower) and (code[i-4] in lower):
                print code[i]
